...
var Grid = (function() {
	...
	function initItemsEvents( $items ) {
		$items.on( 'click', '.btn-close', function() {
			hidePreview();
			return false;
	...
	GridContent = {
		...
		slider: function(){
			if($('.expander .slider').length>0) {
				...
				// update view of the slide
				$slider.on('afterChange', function(event, slick, currentSlide, nextSlide){
					var url_current = $('.is-open .expander .slider li.slick-current.slick-active').find('img').attr('src');
					var title_current = $('.is-open .expander .slider li.slick-current.slick-active').find('.caption').text();
					
					// update image
					$('.is-open .gallery > figure img').attr('src', url_current);
					// update title for demo - should update the full product description and nutricional fact
					$('.is-open .product-title').text(title_current);
		...
		recipeCaptions: function(){
			if($('.list-recipes figcaption').length>0) {
				$('.list-recipes figcaption').matchHeight();
			}
		}
	};
})(jQuery);
